...
def show(spark):
    ...
    st.markdown("#### :red[-] Si visualizza la distribuzione degli hashtags per ogni :red[giorno] (è possibile specificare la categoria) ")
    categoria = st.radio(
    ...
    if bottone2:
        with st.spinner("Disegnando il grafico"):
            result = spark.query.hashtags_distribution_for_category(categoria)
            st.line_chart(result, x="Giorno", y= "hashtags per giorno")
